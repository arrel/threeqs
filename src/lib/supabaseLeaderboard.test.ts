...
describe("Supabase leaderboard", () => {
  ...
  it("groups student names case-insensitively and respects their awarded medals", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async () =>
        jsonResponse([
          { student_name: "Bob", total_score: 330, medal: "silver", date_key: "2026-06-24" },
    ...
    await expect(fetchLeaderboard()).resolves.toEqual([
      { studentName: "Bob", totalPoints: 570, gold: 1, silver: 1, bronze: 0 },
      { studentName: "Ada", totalPoints: 150, gold: 1, silver: 0, bronze: 0 }
    ]);
  });
});
